...
[RFC 6849](https://datatracker.ietf.org/doc/html/rfc6749)
...
그러나, 많은 선택적 구성요소를 가지며 풍부하고 높은 확장성을 지닌 프레임워크이기 때문에, OAuth 2.0 명세 자체만으로는 호환되지 않는 다양한 구현체들이 만들어질 수 있다. 

추가적으로, 이 명세는 몇몇 필수 구성 요소(클라이언트 등록, 인가서버 기능, 엔드포인트 탐색)를 부분적으로 또는 완전히 정의되어 있지 않다.
이러한 구성요소 없이는, 클라이언트는 상호 운용을 위해 특정 인가 서버 및 자원서버에 맞춰 수동으로 구성되어야 한다.

이 프레임워크는 웹 스케일의 완전한 상호 운용성을 달성하는 데 필요한 규범적 프로파일과 확장 기능이 향후 정의될 것이라는 분명한 전체 하에 설계되었다.

> OAuth 2.0에서 명세는 매우 유연하고 일반적 이기 때문에, 이 일반 적인 명세를 특정 상황에 맞게 구체화 한것이 프로파일이다. 즉 프로파일이란 모바일 앱을 위한 OAuth Profile, 웹 앱을 위한 OAuth Profile 등으로 말할 수 있다.

## 1.9. 표현적 관행::representational-conventions

이 사양의 "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "MAY" 그리고 "OPTIONAL" 키워드는
[RFC 2119](https://datatracker.ietf.org/doc/html/rfc2119)에 정의된 내용으로 설명한다.

이 명세는 [RFC 5234](https://datatracker.ietf.org/doc/html/rfc5234)의 Augmented Backus-Naur Form(ABNF) 표기법을 사용한다.
추가적으로 URI 참조 규칙은 [RFC3986](https://datatracker.ietf.org/doc/html/rfc3986) 일반 구문을 참조한다.

Certain security-related terms are to be understood in the sense defined in [RFC4949].  

확실한 보안 관련 문서는 [RFC 4949]에 정의된 것과 같이 이해되어야 한다. 

특정 보안 관련 용어들은 RFC 4949에 정의된 의미로 이해해야 한다. 이러한 용어들은 "attack", "authentication", "authorization", "certificate",
"confidentiality", "credential", "encryption", "identity", "sign", "signature", "trust", "validate", and "verify".를 포함하지만 이에 국한되지 않는다.

별도로 명시되지 않는 한, 모든 프로토콜 파라미터 이름과 값은 대소문자를 구분한다


* MUST: 절대적으로 요구되는 사항을 나타낸다. 해당 조건이 반드시 충족되어야 함을 의미한다.
* MUST NOT: 절대적으로 금지되는 사항을 나타낸다. 해당 조건이 절대로 발생해서는 안 됨을 의미한다.
* REQUIRED: MUST와 동일하게 사용되며, 필수적인 요구 사항을 나타낸다.
* SHALL: MUST와 유사하게 강한 요구 사항을 나타내지만, 규격서에 따라 다르게 해석될 수 있다.
* SHALL NOT: MUST NOT과 유사하게 강한 금지 사항을 나타낸다.
* SHOULD: 권장되는 사항을 나타낸다. 특정 상황에서는 이 규칙을 따르지 않아도 되지만, 그럴 경우에는 그 이유를 충분히 설명할 수 있어야 한다.
* SHOULD NOT: 특정 상황에서 피해야 하는 사항을 나타내며, 마찬가지로 예외적인 경우에는 그 이유를 설명할 수 있어야 한다.
* RECOMMENDED: SHOULD와 동일하게 사용되며, 권장 사항을 나타낸다.
* MAY: 선택적인 사항을 나타내며, 해당 조건을 따라도 되고 안 따라도 되는 경우를 한다.
* OPTIONAL: MAY와 동일하게 사용되며, 선택 사항을 나타낸다.


## 2. 클라이언트 등록::client-registration

OAuth 2.0 프로토콜을 시작하기전에 클라이언트는 인가서버에 등록 해야한다.
클라이언트를 인가서버에 등록하는 방법은 이 사양의 범위를 벗어나지만, 일반적으로 HTML 등록 폼을 통한 최종 사용자의 상호작용을 포함한다.

클라이언트 등록은 클라이언트 와 인가서버간 직접적인 상호작용을 필요로 하지 않는다.
인가 서버가 지원하는 경우, 등록은 신뢰 관계를 수립하고 필요한 클라이언트(예: 리다이렉션, URI, 클라이언트 타입) 속성을 얻기 위한 다른 대상에 의존한다.
예를 들어, 등록은 자체 발행 또는 3자 발행 신원 증명을 이용하거나, 인가 서버가 신뢰할 수 있는 채널을 통해 클라이언트 탐색을 수행함으로써 완료 될 수 있다.

클라이언트를 등록할 때, 클라이언트 개발자는 다음을 반드시 수행해야 한다:

* [섹션 2.1](https://datatracker.ietf.org/doc/html/rfc6749#section-2.1)에 나온 클라이언트 유형을 선택 해야한다.
* [섹션 3.1.2](https://datatracker.ietf.org/doc/html/rfc6749#section-3.1.2)에 나온 리다이렉션 URI를 제공 해야한다.
* 인가 서버에 요구된 모든 다른 정보(예: 어플리케이션 이름, 웹사이트, 설명, 로고 이미지, 법적 약관 동의)를 포함 해야한다.

## 2.1. 클라이언트 유형::client-types

OAuth는 인가 서버와 안전하게 인증하는 기능(즉, 클라이언트 자격 증명의 기밀성을 유지하는 기능)에 따라, 두가지 클라이언트 유형을 정의한다.

**기밀 (confidential)**

자격 증명의 기밀성을 유지할 수 있는 클라이언트(예: 클라이언트 자격 증명에 대한 접근이 제한된 보안 서버에 구현된 클라이언트) 
또는 다른 방법을 사용하여 안전한 클라이언트 인증이 가능한 클라이언트.

**공개 (public)**

자격 증명의 기밀성을 유지할 수 없는 클라이언트 (예: 자원 소유자가 사용하는 장치에서 실행되는 클라이언트, 설치된 네이티브 어플리케이션 또는 웹브라우저 기반 어플리케이션), 
다른 모든 방법으로 안전한 클라이언트 인증을 수행할 수 없는 클라이언트.

클라이언트 유형 설계는 인증서버의 안전한 인증의 정의와 클라이언트 자격 증명의 수용 가능한 노출 수준에 기반된다.
인가 서버는 클라이언트 유형에 대해 추정하지 않는것이 좋다.

클라이언트는 서로 다른 클라이언트 유형과 보안 컨텍스트를 가진 분산된 구성요소의 집합으로 구현될 수 있다.(예: 기밀 서버 기반 구성요소와 공개 브라우저 기반 구성요소를 모두 포함하는 분산 클라이언트).   
인증서버가 특정 클라이언트에 대해 지원을 제공하지 않거나 등록에 대한 지침을 제공하지 않는다면, 클라이언트는 각 구성요소를 별도의 클라이언트로 등록하는 것이 권장된다. 

이 명세는 다음의 클라이언트 프로필을 중심으로 설계되었다:

**웹 애플리케이션(web application)**

웹 애플리케이션은 웹서버에서 동작하는 기밀 클라이언트이다. 자원 소유자는 자원 소유자가 사용하는 장치에서 user-agent에 렌더링된 HTML 사용자 인터페이스를 통해 클라이언트에 접근한다.
클라이언트의 자격증명이며 클라이언트에 발행된 모든 액세스토큰은 웹 서버에 저장되고 자원소유자에게 노출되거나 접근되지 않는다.


**사용자 에이전트 기반 어플리케이션(user-agent-based application)**

사용자 에이전트 기반 애플리케이션은 웹 서버로 부터 클라이언트 코드를 다운로드 받아, 자원 소유자가 사용하는 장치의 유저 에이전트(예: 웹브라우저) 내에서 실행하는 공개 클라이언트이다.
프로토콜 데이터와 자격증명은 자원소유자에 쉽게 접근이 가능(종종 볼수 있으며)하다. 이러한 어플리케이션은 사용자 에이전트 내에 존재하기 때문에, 인가를 요청할 때 사용자 에이전트의 기능들을 원할 하게 사용할 수 있다.


**네이티브 애플리케이션(native application)**

네이티브 애플리케이션은 자원 소유자가 사용하는 장치에 설치되고 실행되는 공개 클라이언트이다.
프로토콜 데이터와 자격 증명은 자원 소유자가 접근 가능하다. 애플리케이션에 포함된 모든 클라이언트 인증 자격증명 추출될 수 있다고 가정한다.
반면에, 액세스 토큰이나 리프레시 토큰과 같은 동적 발행 자격증명은 허용 가능한 수준의 보호를 받을수 있다.
최소한, 이러한 자격 증명은 애플리케이션이 통신할 수 있는 악의적인 서버로 부터 보호된다.
어떤 플랫폼에서는 이러한 자격 증명은 같은 장치에 존재하는 다른 애플리케이션으로부터 보호될 수 있을 것이다.


## 2.2 클라이언트 식별자::client-identifier

인가 서버는 등록된 클라이언트에게 클라이언트 식발자를 발급한다. 즉 고유한 문자열은 클라이언트가 제공한 등록 정보를 나타낸다.
클라이언트 식별자는 비밀이 아니다. 자원 소유자에게 공개되며 클라이언트 인증을 위해 단독 사용되어서는 안된다. 클라이언트 식별자는 인가 서버에 대해 고유하다.

클라이언트 식별자 문자열 크기는 이 명세에서 정의하지 않는다. 클라이언트는 식별자 크기에 대해 추정을 내리는것을 방지 해야만한다. 인증서버는 자신이 발급하는 모든 식별자의 크기를 문서화해야 한다.
